          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {!isAdmin && (
              <button onClick={() => setIsCartOpen(true)} className="relative flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-2xl hover:bg-blue-700 transition shadow-lg font-bold text-sm">
                ...
                Keranjang
                {cartItems.length > 0 && (
                  <span className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center bg-red-500 text-white text-[10px] font-black rounded-full border-2 border-white animate-bounce">
                    {cartItems.length}
                  </span>
                )}
              </button>
            ...
            {isAdmin && (
              <>
                <Link href="/admin/categories" className="bg-blue-50 text-blue-700 px-5 py-3 rounded-2xl hover:bg-blue-100 transition font-bold text-sm border border-blue-100">Kelola Kategori</Link>
                <Link href="/admin/birds/archived" className="bg-gray-800 text-white px-5 py-3 rounded-2xl hover:bg-gray-900 transition font-bold text-sm flex items-center gap-2"><Archive size={18} /> Arsip</Link>
                <Link href="/admin/birds" className="bg-green-600 text-white px-6 py-3 rounded-2xl hover:bg-green-700 transition font-bold text-sm flex items-center gap-2 shadow-lg"><Plus size={20} /> Tambah Burung</Link>
              </>
            )}
          </div>
        </div>
      ...
      <div className={`fixed right-0 top-0 h-screen w-[380px] bg-white shadow-2xl z-[70] flex flex-col transition-transform duration-500 ease-in-out ${isCartOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-6 border-b flex items-center justify-between bg-blue-900 text-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-black tracking-tight">Keranjang Anda</h2>
          </div>
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t bg-white shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
            <div className="flex justify-between mb-6">
              ...
              <span className="text-2xl font-black text-blue-900">Rp {totalHarga.toLocaleString("id-ID")}</span>
            </div>
